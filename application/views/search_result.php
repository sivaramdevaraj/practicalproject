            ...
            <div class="row  prdct_sec">
               <div class="col-md-12">
                  <ol class="breadcrumb breadc_b">
                     <li><a href="<?php echo site_url(); ?>"  class="black"> <i class="fa fa-home"></i> <sapn class="hidden-xs">Home</sapn></a></li>                    
                     <li><a  class="black">Search</a></li>
                     <li><a  class="red"><?=$search_data?></a></li>
                  </ol>
                 <?php if(isset($all_categories) && is_array($all_categories) && count($all_categories)): $i=1;?>
                   <?php foreach ($all_categories as $key => $category) { ?>
                      <div class="panel panel-default pnl_big">
                         <div class="panel-heading">
                            <h4 id="t_<?=str_replace(' ','-',$category->name); ?>" class="text-center"><?=$category->cat_name; ?> - <?=$category->name; ?></h4>
                         </div>
                  ...
                  <?php } ?>
                <?php else : ?><div><h6 align="center">No Results found for "<?=$search_data?>"</h6></div>
                <?php endif; ?>                
               </div>
            </div>
...
$(document).ready(function(){
  ...
  <?php if($this->session->flashdata('login_pop')){?>
    $('#signin2').modal({
       keyboard: false,
       show:true,
       backdrop:'static'
